好的，我们来为 Agent 增加 **工具使用/函数调用 (Tool Use / Function Calling)** 的能力。这将使 Agent 不仅仅依赖 LLM 的内部知识，还能与外部环境交互，执行命令，获取实时信息，从而完成更复杂的任务。
...
我们将主要修改 `dockerfiles/agent/agent.py`。

### 1. 更新 `dockerfiles/agent/requirements.txt` (如果需要)

目前不需要额外添加，`openai` 库已支持函数调用。

### 2. 更新 `dockerfiles/agent/agent.py`
...
这是一个重要的进步，让 Agent 变得更加动态和有能力！记住，工具的设计和LLM如何被引导使用它们是成功的关键。
